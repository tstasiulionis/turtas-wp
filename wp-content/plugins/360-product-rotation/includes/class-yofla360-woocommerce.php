...
class YoFLA360Woocommerce
{
	...
	public function yofla_360_save_product($post_id)
	{
		...
		if (isset($_POST["_y360path_url"])) {
			update_post_meta($post_id, "_y360path_url", sanitize_text_field($_POST["_y360path_url"]));
		}
	...
	public function yofla_360_replace_product_image($content)
	{
		...
		if (false == $this->_is_360_view()) {
			...
			if ($this->_is_variable_product()) {
				 ...
				 * Variable Product
				 */
				$new_content = $this->_get_360player_variableProduct_code();
			} else {
				/**
				 * Simple Product
				 */
				...
				$new_content = $this->_get_360player_embed_code($path);
			}
		}
	 ...
	 * @return bool|string
	 */
	private function _get_360player_embed_code($_y360path)
	{
	 ...
	 * @return string
	 */
	private function _get_360player_variableProduct_code()
	{
		global $post;
