...
;(function ($, window, document, undefined) {
  ...
  var fixThumb = function () {
    ...
    if (element.length == 0) {
      ...
      // set thumb image
      if (element && element.attr && element.attr('src') == data) {
        // all set
        return;
      }
      else {
        // set src
        element && element.attr && element.attr('src', data);
        // do check
        setTimeout(function () {
          fixThumb.apply(_scope)
        }, 500)
  ...
  $(function () {
    ...
    if (typeof wc_add_to_cart_variation_params !== 'undefined') {
      ...
      some_form && some_form[0] &&
      showVariantByProductId(some_form[0]['variation_id']);
    }
